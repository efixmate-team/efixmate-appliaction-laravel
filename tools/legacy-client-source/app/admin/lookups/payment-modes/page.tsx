...
export default function PaymentModesPage() {
  ...
  const handleToggle = async (newValue: boolean, row: any) => {
    try {
      if (newValue) {
        await masterAPI.updateLookup(resource, row.payment_mode_id, { is_active: true });
        fetchData();
      } else {
        setToggleRecordId(row.payment_mode_id);
  ...
  const handleUpdateSubmit = async () => {
    ...
    try {
      ...
      const response = await masterAPI.updateLookup(resource, editRow.payment_mode_id, payload);
  ...
  return (
    <div className="space-y-6">
      ...
      <Form
        ...
        cols={2} card title="Add Payment Modes" subtitle="Create a new entry"
      ...
      >
        <Input name="payment_mode" title="Mode Name" required />
        <Input name="description" title="Description" required />
      ...
      {editRow && (
        ...
        >
          <Input name="payment_mode" title="Mode Name" required value={editForm.payment_mode} onChange={(e) => setEditForm(p => ({...p, payment_mode: e.target.value}))} />
          <Input name="description" title="Description" required value={editForm.description} onChange={(e) => setEditForm(p => ({...p, description: e.target.value}))} />
      ...
      <PaginatedTable
        ...
        title="Payment Modes" badge="Settings" subtitle="System records"
        ...
        showAdd={false} showExport showFilter filters={tableFilters} showRefresh onRefresh={() => fetchData()} rowKey="payment_mode_id"
        headerActions={
      ...
      >
        ...
        <Column header="Mode Name" dataKey="payment_mode" sortable />
      ...
      <BulkUploadModal
        ...
        templateFileName="payment_modes_template.xlsx"
        columns={["payment_mode", "description", "is_active"]}
        exampleRow={["Cash", "Cash payment", "TRUE"]}
        columnDescription="payment_mode, description, is_active (TRUE/FALSE)"
        onUpload={async (rows) => {
