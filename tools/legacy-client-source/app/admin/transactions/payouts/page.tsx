...
const PAYOUT_STATUS_OPTIONS: FilterOption[] = [
...
export default function PayoutsPage() {
  ...
  const displayData = useMemo(() => {
    ...
    return filterBySearch(rows, search, ["first_name", "last_name"]);
  }, [activeFiltered, statusFilter, search]);

  const tableFilters = useMemo(
    () => [
      ...activeFilters,
      dropdownFilterDescriptor(statusFilter, setStatusFilter, "All Statuses", PAYOUT_STATUS_OPTIONS),
    ],
  ...
  const handleToggle = async (newValue: boolean, row: any) => {
    try {
      await masterAPI.updateLookup(resource, row.payout_id, { is_active: newValue });
      fetchData();
    } catch (err) {}
  };
  ...
  const handleUpdateSubmit = async () => {
    ...
    try {
      ...
      const response = await masterAPI.updateLookup(resource, editRow.payout_id, payload);
  ...
  return (
    <div className="space-y-6">
      ...
      <Form
        ...
        cols={2} card title="Add Payout" subtitle="Manage technician payouts"
      ...
      >
        <AsyncSelect name="technician_id" title="Technician" resource="technicians" labelKey="first_name" required />
        <Input name="amount" title="Amount" required type="number" />
        <Select name="status" title="Status" options={[{value: 'PENDING', label: 'Pending'}, {value: 'PROCESSED', label: 'Processed'}]} required />
      ...
      {editRow && (
        <Form
          showMe={openEditForm}
          cols={2} card title="Edit Payout" subtitle="Modifying record"
          onSubmit={handleUpdateSubmit}
          onReset={() => { setOpenEditForm(false); setEditRow(null); }}
          submitLabel="Save Changes" showReset loading={editLoading}
        >
          <AsyncSelect
            name="technician_id" title="Technician" resource="technicians" labelKey="first_name" required
            value={editForm.technician_id} onChange={(e: any) => setEditForm(p => ({...p, technician_id: e.target.value}))}
          />
          ...
          <Select
            name="status" title="Status" options={[{value: 'PENDING', label: 'Pending'}, {value: 'PROCESSED', label: 'Processed'}]} required
      ...
      <PaginatedTable
        ...
        title="Payouts" badge="Transactions" subtitle="Payout records"
        ...
        showSearch showFilter showExport exportFileName="payouts" filters={tableFilters}
        showAdd showRefresh addLabel="Add New" onAdd={() => setOpenCreateForm(true)} onRefresh={() => fetchData()} rowKey="payout_id"
      >
