/**
 * replace-colors.mjs
 * Replaces all Tailwind named color classes with explicit hex arbitrary-value equivalents.
 * e.g.  bg-blue-600  →  bg-[#2563eb]
 *       hover:text-gray-500  →  hover:text-[#6b7280]
 *       border-red-200/50   →  border-[#fecaca]/50
 */
...
// ── Full Tailwind v3 default palette ──────────────────────────────────────────
const HEX = {
  // Slate
...
// Special single-word colors
const SPECIAL = { white: '#ffffff', black: '#000000' };

// CSS utility prefixes that accept a color value
const PREFIXES = [
  'bg','text','border','ring','ring-offset',
  'from','to','via',
  'fill','stroke',
  'placeholder','decoration','outline','shadow','caret','divide','accent',
];

// Build regex that matches: (modifiers:)?(prefix)-(color-shade)(/opacity)?
// e.g. hover:md:bg-blue-600/50
const COLOR_KEYS  = Object.keys(HEX).join('|');
const SPECIAL_KEYS = Object.keys(SPECIAL).join('|');
const PREFIX_ALT  = PREFIXES.map(p => p.replace('-','\\-')).join('|');

// Matches the color portion: blue-600  OR  white / black
const COLOR_PART  = `(?:${COLOR_KEYS}|${SPECIAL_KEYS})`;
// Matches optional opacity: /50  /75  /100
const OPACITY     = `(?:\\/\\d+)?`;
// Matches optional modifier chain: hover: focus: md: dark: group-hover: etc.
const MODIFIERS   = `(?:[\\w\\-]+:)*`;

const PATTERN = new RegExp(
  `(${MODIFIERS})(${PREFIX_ALT})-(${COLOR_PART})(${OPACITY})`,
  'g'
);

function replaceContent(content) {
  return content.replace(PATTERN, (match, mods, prefix, color, opacity) => {
    const hex = HEX[color] ?? SPECIAL[color];
    if (!hex) return match;
    return `${mods}${prefix}-[${hex}]${opacity}`;
  });
}

// ── Walk directory ─────────────────────────────────────────────────────────────
const SKIP = new Set([
  'node_modules', '.next', '.next-build-2', '.git', '.turbo',
  'dist', 'out', 'build', 'scripts',
]);
...
for (const file of files) {
  ...
  const replaced  = replaceContent(original);
  if (replaced !== original) {
    fs.writeFileSync(file, replaced, 'utf8');
    changed++;
    console.log(`  ✔ ${path.relative(CLIENT_DIR, file)}`);
  } else {
    unchanged++;
  }
}

console.log(`\nDone — ${changed} files updated, ${unchanged} files unchanged.`);
